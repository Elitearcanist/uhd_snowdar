...
def load_radar_data(
    ...
    max_file_size_bytes = (
        rx_len_samples
        * int(1 / config["CHIRP"]["pulse_rep_int"])
        * 8
        * max_seconds_to_load
    )
    load_start_bytes = (
    ...
    if file_size_bytes > max_file_size_bytes:
        print(
            f"WARNING: File is {file_size_bytes/(2**30):.2f} GB ({file_size_bytes / (rx_len_samples*int(1/config['CHIRP']['pulse_rep_int'])*2):.2f} seconds). Only loading the first {max_seconds_to_load} seconds."
        )
        file_size_bytes = max_file_size_bytes

    rx_sig = np.zeros((file_size_bytes // 8,), dtype=np.csingle)
    for start_offset in np.arange(
        0, file_size_bytes, max_chunk_size_samples * 8, dtype=np.int64
    ):
        if start_offset + max_chunk_size_samples * 8 > file_size_bytes:
            rx_sig[(start_offset // 8) :] = extractSig(
                rx_samps,
                count=(file_size_bytes - start_offset) // 4,
                offset=load_start_bytes + start_offset,
            )
        else:
            rx_sig[
                (start_offset // 8) : ((start_offset // 8) + (max_chunk_size_samples))
            ] = extractSig(
                rx_samps,
                count=max_chunk_size_samples * 2,
                offset=load_start_bytes + start_offset,
            )
    ...
    rx_sig_reshaped = np.transpose(
        np.reshape(rx_sig, (n_rxs, rx_len_samples), order="C")
    )
    ...
    slow_time = np.linspace(
        0,
        config["CHIRP"]["pulse_rep_int"] * config["CHIRP"]["num_presums"] * n_rxs,
        np.shape(rx_sig_reshaped)[1],
    )
    ...
    if os.path.exists(log_file):
        ...
        print(
            f"WARNING: No log file found. This is fine, but checks for error codes will be disabled."
        )
    ...
    if errors is None:
        if error_behavior is not None:
            print(
                f"WARNING: Requested doing something with errors but no log file was loaded. Defaulting to doing nothing."
            )
...
def extractSig(filename, count=-1, offset=0):
    sig_floats = np.fromfile(
        filename, dtype=np.float32, count=count, sep="", offset=offset
    )
...
def loadSamplesFromFile(
    ...
    for start_offset in np.arange(0, file_size_bytes, max_chunk_size, dtype=np.int64):
        if start_offset + max_chunk_size > file_size_bytes:
            ...
            rx_sig[(start_offset // 8) :] = extractSig(
                filename,
                count=file_size_bytes - start_offset,
                offset=load_start_bytes + start_offset,
            )
        else:
            # print(f"{max_chunk_size}, {start_offset}")
            rx_sig[
                (start_offset // 8) : ((start_offset // 8) + (max_chunk_size // 2))
            ] = extractSig(
                filename, count=max_chunk_size, offset=load_start_bytes + start_offset
            )
...
def stack(radar_data, n):
    radar_data_stacked = np.zeros_like(
        radar_data, shape=(np.shape(radar_data)[0], np.shape(radar_data)[1] // n)
    )
    for out_idx, start_idx in enumerate(
        np.arange(0, np.shape(radar_data_stacked)[1] * n, n)
    ):
        radar_data_stacked[:, out_idx] = np.mean(
            radar_data[:, start_idx : (start_idx + n)], axis=1
        )
...
def pulse_compress(radar_data, chirp, fs, upsampling=1, zero_sample_idx=0):
    ...
    xcorr_results = np.zeros(
        (
            ((np.shape(radar_data)[0] * upsampling) - len(corr_sig)) + 1,
            np.shape(radar_data)[1],
        ),
        dtype=np.csingle,
    )

    fast_time = np.linspace(
        0, np.shape(xcorr_results)[0] / (fs * upsampling), np.shape(xcorr_results)[0]
    )
    ...
    for res_idx in range(np.shape(xcorr_results)[1]):
        ...
        xcorr_results[:, res_idx] = scipy.signal.correlate(
            stacked, corr_sig, mode="valid", method="auto"
        ) / np.sum(np.abs(corr_sig) ** 2)
...
def findDirectPath(correl_sig, direct_start, describe=True):
    ...
    if describe:
        print(
            "\tThe direct path peak was found at sample %d with value %f."
            % (dir_peak, np.abs(correl_sig[dir_peak]))
        )
    return dir_peak
...
def findEcho(correl_sig, sample_rate, dir_peak, echo_start, sig_speed, describe=True):
    ...
    if describe:
        print(
            "\tThe strongest echo peak was found at sample %d with value %f."
            % (echo_peak, correl_sig[echo_peak])
        )
...
def getSNR(ideal, actual):
    ...
    if np.shape(ideal)[0] != np.shape(actual)[0]:
        ...
        print(
            "Ideal: %d samples. Actual: %d samples."
            % (np.shape(ideal)[0], np.shape(actual)[0])
        )
        return
